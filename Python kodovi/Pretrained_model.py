...
# cv.imwrite('input1.png', cv.cvtColor(test1, cv.COLOR_RGB2BGR))
# cv.imwrite('input2.png', cv.cvtColor(test2, cv.COLOR_RGB2BGR))
# cv.imwrite('input3.png', cv.cvtColor(test3, cv.COLOR_RGB2BGR))
...
cv.imwrite('input4.png', cv.cvtColor(my_img, cv.COLOR_RGB2BGR))
...
for layer in model.model.layers:
    ...
    if str(layer).find('Conv2D') != -1:
        ...
        WriteBias(layer.weights[1].numpy(), '../Tezine/bias' + str(cnt) + '.txt')
        WriteWeights(layer, '../Tezine/weights' + str(cnt) + '.txt')
        cnt += 1
